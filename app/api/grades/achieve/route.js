...
export async function POST(request) {
  try {
    ...
    if (existingGrade.recordset.length > 0) {
      // Grade already exists, just return success with the existing grade ID
      return NextResponse.json({
        success: true,
        grade_id: existingGrade.recordset[0].id,
        message: `Grade ${grade_number} already achieved`
      });
    }
    ...
    try {
      ...
      // Link completions to this grade if provided
      if (completion_ids && Array.isArray(completion_ids) && completion_ids.length > 0) {
