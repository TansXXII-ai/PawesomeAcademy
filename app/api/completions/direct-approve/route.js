...
export async function POST(request) {
  try {
    ...
    // Only trainers and admins can directly approve
    if (!['trainer', 'admin'].includes(currentUser.role)) {
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    }

    const body = await request.json();
    const { user_id, skill_id, notes } = body;
    ...
    // Get skill info for the response
    const skillResult = await query(
      'SELECT title, points FROM Skills WHERE id = @skillId',
      [{ name: 'skillId', type: 'Int', value: skill_id }]
    );
    ...
    try {
      ...
      const completionResult = await transaction.request()
      ...
      const completionId = completionResult.recordset[0].id;

      // Create a submission record for tracking (optional but recommended)
      await transaction.request()
        .input('userId', sql.Int, user_id)
        .input('skillId', sql.Int, skill_id)
        .input('trainerId', sql.Int, currentUser.id)
        .input('notes', sql.NVarChar, notes || 'Approved in-class by trainer')
        .query(`
      ...
      return NextResponse.json({
        ...
        skill: skill.title,
        points: skill.points,
        message: `${skill.title} approved! +${skill.points} points`
      });
